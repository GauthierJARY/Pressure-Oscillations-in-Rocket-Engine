# -*- coding: utf-8 -*-
"""
Created on Mon Jul 24 11:31:33 2023
...
def mu_compos(m1,m2):
    if m2 == 0 : # debil case for initial conditions
        mu_ox_burned, mu_ox_out, mu_fu_burned, mu_fu_out = 1,1,1,1
    else:
        MRin = m1/m2
        MRst = 0.5 * 32/2 # molar stoechio : 2 H2 + O2 = 2 H2O
        # we put it for mass stoechio thanks to molar mass of each species
        if MRin <= MRst: # cf Perez Rocca expressions
...
while t<0.6:
    ...
    list_mixture_ratio_inside.append( (mu_compos(m1, m2)[0] * m1/32) / (mu_compos(m1, m2)[2] * m2/2) )
