...
#   ==> this code used all the improvements we made except the Mixture Ratio !!!
# prediction of temperature is then used with the incoming mass flow, which was not the example detailled in the report ! 
# Mixture ratio of the explicit code version must be implemented here but there is a need to do few modifications in the code structure
# =============================================================================
...
material_pipe = 50e9 #Pa  Young Modulus material of the wall aluminium
material_dome = 50e9
...
def a_sound(density, bulk_modulus,diameter, material):
    ...
    return  bulk_modulus / density # non corrected
...
# We have to call for Cantera for prediction and initialisation
...
N= len(x) # Calcul du nombre d'échantillon
X_abs = np.abs(X[:N//2])*2.0/N # On prend la valeur absolue de l'amplitude uniquement pour les fréquences positives et normalisation
freq_pos = freq[:N//2] # On garde uniquement les fréquences positives
